...
class cdriscv(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","cdriscv")
    ...
    def initialise(self, suite, work_dir, archtest_env):
       ...
       # runTests function
       # The repository root, so that the plugin can reach the bench and
       # the image builder without depending on the caller's cwd.
    ...
    def build(self, isa_yaml, platform_yaml):
      ...
      # for cdriscv start building the '--isa' argument. the self.isa is dutnmae specific and may not be
      # useful for all DUTs
    ...
    def runTests(self, testList):
	  ...
	  # echo statement.
          if self.target_run:
            ...
            #             0x80000000, so code, .tohost and the signature
            #             region all land at the right word offsets
            #   mkimage   binary -> the 39-bit ECC hex image the TCM
            #             loads, which is the same builder the rest of
            #             the suite uses
            #   vvp       run, watching for the store to tohost, then
            #             dump the signature words
            #
