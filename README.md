...
| Gate level simulation (`make gate`) | **3 blocks pass** | objective O8 begun: ALU, multiplier and SEC-DED synthesised to real IHP SG13G2 cells, same benches re-run on the netlist, plus an illegal-state recovery check that re-argues waiver W2a against the gates. Functional only — every delay in the library is zero, so this says nothing about timing |
