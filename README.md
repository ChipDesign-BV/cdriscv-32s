...
| Architectural test suite (`make riscof`) | **62 of 62 pass** | **objective O1 met.** `riscv-arch-test` **3.5.3**, unmodified — the newest release predating the `c.nop` alignment padding that current releases emit and an RV32I core must trap on. 38 I, 8 M, 15 privilege, 1 Zifencei. The three misaligned-load failures reported at V34 were a defect in the bench environment, not the core: see V35 in [verification_findings.md](doc/verification_findings.md) |
| Static timing (`make sta`) | **hold met; depth is not the limit** | OpenSTA against the SG13G2 library. Hold worst slack **+0.184 ns**. Setup cannot be turned into an Fmax before buffer insertion, but the two effects are now separated: on the worst logic path, 7 unbuffered cells take 32.4 ns and the other **47 gates take 7.0 ns** (0.149 ns each). **The pipeline does not need re-splitting — what is missing is a buffer tree**, which is a place-and-route job. A workload that re-reads the configuration each iteration takes latent from 46.4 % to **4.3 %** (`make fi-check`), at roughly double the run time. See [verification_findings.md](doc/verification_findings.md) |
...
| Fault injection (`make fi`) | **2 600 upsets, 26 elements** | **0 silent data corruption, 0 hangs** — but **46 % latent**: 1 207 runs finished correctly with a safety mechanism silently disabled, and **12 configuration registers are 100 % undetected**. Detection fell 41.0 → 31.9 → 24.8 % as the fault list grew 9 → 20 → 26 with no design change, which is what a diagnostic figure quoted without its fault list is worth. See [verification_findings.md](doc/verification_findings.md) |
