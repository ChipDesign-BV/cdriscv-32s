...
| Architectural test suite (`make riscof`) | **not run** | objective O1. RISCOF, the official `riscv-arch-test` suite, the target environment and a DUT plugin driving the Icarus bench are all in place — the Spike reference now produces **128 signatures**, and the DUT executes the tests — but the released suite pads with `c.nop` unconditionally and this core has no C extension, so it traps on the padding. **No conformance result exists.** See [verif/riscof/README.md](verif/riscof/README.md) |
