...
| Static timing (`make sta`, `make fmax`) | **75.9 MHz placed and buffered** | `make fmax` synthesises with the TCMs mapped to four real IHP SRAM macros, then OpenROAD floorplans, places and repairs (5 955 buffers, 917 resizes) before reporting: worst setup slack **-3.18 ns at 10 ns**, typ corner, 2.58 mm² at 46 % utilisation. The critical path is the **64-bit `minstret` single-cycle ripple increment** — not ALU, not ECC — and the staged-carry fix is design work recorded in V38. Below the 100 MHz target until then. Hold (unbuffered check) **+0.184 ns** |
