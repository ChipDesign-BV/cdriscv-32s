...
| Architectural test suite (`make riscof`) | **59 of 62 pass** | objective O1 has a result. `riscv-arch-test` **3.5.3**, unmodified — the newest release predating the `c.nop` alignment padding that current releases emit and an RV32I core must trap on. Three failures, all misaligned **loads** (`lh`, `lhu`, `lw`), while misaligned stores pass: **open and unexplained**, see [verification_findings.md](doc/verification_findings.md) |
