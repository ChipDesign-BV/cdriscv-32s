...
| Architectural test suite (`make riscof`) | **not run** | objective O1. RISCOF, the official `riscv-arch-test` suite, the target environment and a DUT plugin driving the Icarus bench are all in place — but the Spike **reference** model runs far too slowly here for a run to complete, so **no conformance result exists**. See [verif/riscof/README.md](verif/riscof/README.md) |
